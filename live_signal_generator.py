...
# Score bands per STRATEGY_SPEC.txt
BAND_A_LO, BAND_A_HI = 92.0, 95.0   # 2x leverage
BAND_B_LO, BAND_B_HI = 98.0, 100.0  # 1x leverage
...
def apply_filters(df: pd.DataFrame, regime: str, top_n: int) -> pd.DataFrame:
    ...
    # Band A: score 92-95 → 2x leverage
    band_a = df[
    ...
    # Band B: score 98-100 → 1x leverage
    band_b = df[
    ...
    # Top-N by score across both bands
